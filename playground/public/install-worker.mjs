    ...
    if (action === 'start') {
        await navigator.locks.request('start', async () => {
            ...
            if (checkWww.exists) {
                ...
                const initPhpCode = fetch('/assets/init.php-wasm');
                await php.binary;
                ...
                const installSiteCode = await (await fetch('/assets/install-site.php-wasm')).text();
        ...
        await self.navigator.locks.request('export', async () => {
            ...
            const exportPhpCode = fetch('/assets/export.php-wasm');
            await php.binary;
